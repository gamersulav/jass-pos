...
async function computeClosingForDate(db, date, depth = 0) {
  ...
  const open = {}, adj = {}, sales = {}, expByMethod = {};
  METHODS.forEach(m => { open[m] = 0; adj[m] = 0; sales[m] = 0; expByMethod[m] = 0; });
  ...
  METHODS.forEach(m => {
    ...
    closing[m] = open[m] + sales[m] - out + adj[m];
  });
  return closing;
}
...
export default async function handler(req, res) {
  ...
  const [saleRows, expenseRows, supplierRows] = await Promise.all([
    db.query(`
      SELECT s.id, s.created_at, s.payment_method, s.total_amount as amount,
        GROUP_CONCAT(si.item_name || ' ×' || si.quantity) as items
    ...
    db.query(`SELECT shop_name, quantity*unit_cost as amount, settled_at FROM shop_tabs WHERE direction='in' AND settled=1 AND date(settled_at,'+5 hours','+45 minutes')=?`, [targetDate]),
  ]);
  ...
  const [histSale, histExp, histSupplier] = await Promise.all([
    db.query(`
      SELECT date(created_at,'+5 hours','+45 minutes') as date, COALESCE(SUM(total_amount),0) as total
      FROM sales WHERE date(created_at,'+5 hours','+45 minutes') >= date('now','-30 days','+5 hours','+45 minutes')
      GROUP BY date(created_at,'+5 hours','+45 minutes')
    `),
    db.query(`SELECT expense_date as date, COALESCE(SUM(amount),0) as total FROM expenses WHERE expense_date >= date('now','-30 days','+5 hours','+45 minutes') GROUP BY expense_date`),
    db.query(`
      SELECT date(settled_at,'+5 hours','+45 minutes') as date, COALESCE(SUM(quantity*unit_cost),0) as total
  ...
  const histMap = {};
  histSale.forEach(r => { if (!histMap[r.date]) histMap[r.date] = { date: r.date, sales: 0, expenses: 0, supplier_payments: 0 }; histMap[r.date].sales += Number(r.total); });
  histExp.forEach(r => { if (!histMap[r.date]) histMap[r.date] = { date: r.date, sales: 0, expenses: 0, supplier_payments: 0 }; histMap[r.date].expenses += Number(r.total); });
  histSupplier.forEach(r => { if (!histMap[r.date]) histMap[r.date] = { date: r.date, sales: 0, expenses: 0, supplier_payments: 0 }; histMap[r.date].supplier_payments += Number(r.total); });
