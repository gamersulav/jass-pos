...
function SaleTab({ user }) {
  ...
  function addShoe(shoe, size) {
    const qty = Number(shoe.sizes?.[size] || 0);
    if (!qty) return;
    const existing = cart.findIndex(c => c.shoe_id === shoe.id && c.shoe_size === size);
    if (existing >= 0) {
      const cur = cart[existing];
      if (cur.quantity >= qty) return setMsg('Not enough stock');
      setCart(c => c.map((it,i) => i===existing ? {...it, quantity: it.quantity+1} : it));
    } else {
      setCart(c => [...c, { item_type:'shoe', shoe_id:shoe.id, shoe_size:size, accessory_id:null, item_name:`${shoe.name}${shoe.brand?' - '+shoe.brand:''} (sz ${size})`, quantity:1, unit_price: shoe.selling_price, cost_price: shoe.cost_price||0 }]);
    }
    setMsg('');
  }
  ...
  function addAcc(acc) {
    ...
    if (existing >= 0) {
      ...
      setCart(c => [...c, { item_type:'accessory', shoe_id:null, shoe_size:null, accessory_id:acc.id, item_name:acc.name, quantity:1, unit_price:acc.selling_price, cost_price:acc.cost_price||0 }]);
    }
    setMsg('');
  }
  ...
  async function checkout() {
    ...
    setCart([]); setDiscount(''); setCreditCustomer(''); setMsg(`Sale #${data.saleId} completed!`);
    fetch('/api/shoes').then(r=>r.json()).then(setShoes);
    fetch('/api/accessories').then(r=>r.json()).then(setAccessories);
  }
                ...
                <Card key={shoe.id}>
                  ...
                  <Select label="Size" value={selSize[shoe.id]||''} onChange={e => setSelSize(s => ({...s,[shoe.id]:e.target.value}))}>
                    ...
                    {SIZES.filter(sz => Number(shoe.sizes?.[sz]) > 0).map(sz => (
                      <option key={sz} value={sz}>Size {sz} ({shoe.sizes[sz]} left)</option>
                    ))}
                  </Select>
                  <Btn onClick={() => addShoe(shoe, Number(selSize[shoe.id]))} disabled={!selSize[shoe.id]} style={{ width:'100%' }}>Add to Cart</Btn>
                </Card>
              ))}
            </div>
          </>
...
function StockTab() {
  ...
  const [form, setForm] = useState({ item_type:'shoe', shoe_id:'', accessory_id:'', size:'', quantity:'', notes:'' });
  const [msg, setMsg] = useState('');
  ...
  async function addStock() {
    ...
    const r = await fetch('/api/stock', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ ...form, shoe_id: form.shoe_id ? Number(form.shoe_id) : null, accessory_id: form.accessory_id ? Number(form.accessory_id) : null, size: form.size ? Number(form.size) : null, quantity: Number(form.quantity), item_name, direction:'in' }) });
    const data = await r.json();
    if (!r.ok) return setMsg(data.error || 'Error');
    setMsg('Stock added!');
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'340px 1fr', gap:20 }}>
      <Card>
        ...
        <Select label="Type" value={form.item_type} onChange={e=>setForm(f=>({...f,item_type:e.target.value,shoe_id:'',accessory_id:'',size:''}))}>
          <option value="shoe">Shoe</option>
          <option value="accessory">Accessory</option>
        </Select>
        {form.item_type === 'shoe' ? (
          <>
            <Select label="Shoe" value={form.shoe_id} onChange={e=>setForm(f=>({...f,shoe_id:e.target.value,size:''}))}>
            ...
            <Select label="Size" value={form.size} onChange={e=>setForm(f=>({...f,size:e.target.value}))}>
              ...
              {SIZES.map(sz => <option key={sz} value={sz}>Size {sz}{selShoe?.sizes?.[sz] ? ` (${selShoe.sizes[sz]} in stock)` : ''}</option>)}
            </Select>
          </>
        ) : (
          ...
          <Card key={e.id} style={{ marginBottom:8, padding:12 }}>
            <div style={{ display:'flex', justifyContent:'space-between' }}>
              <div>
                ...
                {e.size && <span style={{ color:C.muted, fontSize:12, marginLeft:8 }}>size {e.size}</span>}
              </div>
